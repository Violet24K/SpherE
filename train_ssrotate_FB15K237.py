...
from openke.module.model import SSRotatE, RotatE
...
def main(args):
    ...
    # not actually used dataloader for training
    ...
    if not args.test:
        # train the model
        trainer = Trainer(model = model, data_loader = train_dataloader, train_times = 300, alpha = 2e-5, use_gpu = True, opt_method = "adam") # normal
        trainer.run()
        rotate.save_checkpoint('./checkpoint/SSrotate_FB15K237.ckpt')

    # test the model
    rotate.load_checkpoint('./checkpoint/SSrotate_FB15K237.ckpt')
    ...
    # tester = TesterForRetrievalTest(model = transe, data_loader = test_dataloader, use_gpu = True)
