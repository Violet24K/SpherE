...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, house_dim, house_num, housd_num, thred,
        ...
        if model_name == 'SSHousE_r':
            self.entity_radius = nn.Parameter(self.embedding_range/10*torch.ones(nentity, self.entity_dim))
            self.marginal_val = 0.1
        ...
        if model_name not in ['HousE_r', 'SSHousE_r']:
    ...
    def forward(self, sample, mode='single'):
        ...
        if mode == 'head-batch':
            ...
            # expect: [14541, 30]-> [500, 500, 30]
            if hasattr(self, 'entity_radius'):
                head_radius = torch.index_select(
                    self.entity_radius,
                    dim=0,
                    index=head_part.view(-1)
            ...
            # expect: [14541, 30] -> [500, 1, 30]
            if hasattr(self, 'entity_radius'):
                tail_radius = torch.index_select(
                    self.entity_radius,
                    dim=0,
                    index=tail_part[:, 2]
                ).unsqueeze(1)
        ...
        elif self.model_name == 'SSHousE_r':
            score = self.SSHousE_r(head, relation, tail, head_radius, tail_radius, mode)
        else:
    ...
    def HousE_r(self, head, relation, tail, mode):
        ...
        if mode == 'head-batch':
            ...
            cos_score = tail - head
            cos_score = torch.sum(cos_score.norm(dim=3, p=1), dim=2)
        else:
            for i in range(self.house_num):
    ...
    def SSHousE_r(self, head, relation, tail, head_radius, tail_radius, mode):
        ...
        if mode == 'head-batch':
            for i in range(self.house_num):
                tail = tail - 2 * (r_list[i] * tail).sum(dim=-1, keepdim=True) * r_list[i]
            score = tail - head
            score = score.norm(dim=3, p=2)
            dist_margin = torch.stack((score - tail_radius - head_radius , -self.marginal_val * head_radius))    # might need to change to head_radius
            score, max_idxs = torch.max(dist_margin, dim=0)
            score = score + self.marginal_val * head_radius
            ...
            for i in range(self.house_num):
                ...
                head = head - 2 * (r_list[j] * head).sum(dim=-1, keepdim=True) * r_list[j]
            score = head - tail
            # [500, 500, 30, 20] -> [500, 500, 30]
            score = score.norm(dim=3, p=2)
            dist_margin = torch.stack((score - head_radius - tail_radius, -self.marginal_val * tail_radius))    # might need to change to head_radius
            # [2, 500, 500, 30] -> [500, 500, 30]
            score, max_idxs = torch.max(dist_margin, dim=0)
            score = score + self.marginal_val * tail_radius
            # [500, 500, 30] -> [500, 500]
            score = torch.sum(score, dim=2)
    ...
    def SSHousE_r_predict(self, head, relation, tail, head_radius, tail_radius, mode):
    ...
    def predict(self, sample, mode='single'):
        ...
        if self.model_name == 'HousE_r':
            ...
            score = self.SSHousE_r_predict(head, relation, tail, head_radius, tail_radius, mode)
        else:
            raise ValueError('model %s not supported' % self.model_name)
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        # negative_score = 5 + (negative_score)
        ...
        # positive_score = 5 + (positive_score)
